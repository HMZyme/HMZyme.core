...
from hmzyme import __version__ as hmzyme_version
from hmzyme.utils import is_valid_dir
from hmzyme.diagram_cycles import create_carbon_cycle, create_nitrogen_cycle, create_sulfur_cycle, create_other_cycle

MESSAGE = '''
Create figures from hmzyme and esmecata outputs.
'''
REQUIRES = '''
...
def compute_hmzyme_functions_occurrence(hmzyme_output_file, tax_id_names_observation_names=None):
    """Read pathway_presence.tsv or function_presence.tsv created by hmzyme to compute the occurrence of each functions/pathways.

    Args:
        hmzyme_output_file (str): path to the output file of hmzyme (either pathway_presence.tsv or function_presence.tsv).
    ...
    """
    hmzyme_function_df = pd.read_csv(hmzyme_output_file, sep='\t')
    hmzyme_function_df.set_index('function', inplace=True)

    all_studied_organisms = hmzyme_function_df.columns
    ...
    for function_name, row in hmzyme_function_df.iterrows():
        all_functions.append(function_name)
        for organism in hmzyme_function_df.columns:
...
def compute_hmzyme_functions_abundance(hmzyme_output_file, sample_abundance, sample_tot_abundance, tax_id_names_observation_names=None):
    """Read pathway_presence.tsv or function_presence.tsv created by hmzyme to compute the occurrence of each functions/pathways.
    ...
    # Compute the occurrence of functions in organism from hmzyme file.
    function_organisms, all_studied_organisms = compute_hmzyme_functions_occurrence(hmzyme_output_file, tax_id_names_observation_names)
...
def visualise_barplot_category(category, gene_categories, df_seaborn_abundance, output_file_name):
    ...
    Args:
        ...
        gene_categories (dict): adicitonary mapping function category to their respective function inferred by hmzyme
        df_seaborn_abundance (pd.DataFrame): dataframe pandas containing a column with the name of function,  a second column with the relative abundance of organisms having it in the community and a third column for the sample
...
def create_visualisation(hmzyme_output, output_folder, esmecata_output_folder=None, abundance_file_path=None):
    """Create visualisation plots from esmecata, hmzyme output folders

    Args:
        hmzyme_output (str): path to hmzyme output folder.
    ...
    logger.info("  -> Read hmzyme cycle output files.")
    hmzyme_pathway_presence_file = os.path.join(hmzyme_output, 'pathway_presence.tsv')
    cycle_occurrence_organisms, all_studied_organisms = compute_hmzyme_functions_occurrence(hmzyme_pathway_presence_file, tax_id_names_observation_names)
    ...
    all_cycles = pd.read_csv(hmzyme_pathway_presence_file, sep='\t')['function'].tolist()
    diagram_data = {}
    ...
    logger.info("  -> Read hmzyme functions output files.")
    hmzyme_function_presence_file = os.path.join(hmzyme_output, 'function_presence.tsv')
    function_occurrence_organisms, all_studied_organisms = compute_hmzyme_functions_occurrence(hmzyme_function_presence_file, tax_id_names_observation_names)
    # Compute the relative abundance of organisms by dividing for a function the number of organisms having it by the total number of organisms in the community.
    ...
    if abundance_file_path is not None:
        ...
        logger.info("  -> Read hmzyme cycle output files.")
        hmzyme_pathway_presence_file = os.path.join(hmzyme_output, 'pathway_presence.tsv')
        cycle_abundance_samples, cycle_relative_abundance_samples, cycle_participation_samples = compute_hmzyme_functions_abundance(hmzyme_pathway_presence_file, sample_abundance, sample_tot_abundance, tax_id_names_observation_names)
        ...
        logger.info("  -> Read hmzyme function output files.")
        hmzyme_function_presence_file = os.path.join(hmzyme_output, 'function_presence.tsv')
        function_abundance_samples, function_relative_abundance_samples, function_participation_samples = compute_hmzyme_functions_abundance(hmzyme_function_presence_file, sample_abundance, sample_tot_abundance, tax_id_names_observation_names)
    ...
    metadata_json['tool_dependencies']['python_package']['hmzyme'] = hmzyme_version
    ...
    metadata_json['input_parameters'] = {'esmecata_output_folder': esmecata_output_folder, 'hmzyme_output': hmzyme_output, 'output_folder': output_folder,
                                         'abundance_file_path': abundance_file_path}
    metadata_json['duration'] = duration

    metadata_file = os.path.join(output_folder, 'hmzyme_visualisation_metadata.json')
...
def main():
    ...
    parent_parser_hmzyme = argparse.ArgumentParser(add_help=False)
    parent_parser_hmzyme.add_argument(
        '--hmzyme',
        dest='hmzyme',
        required=True,
        help='hmzyme output folder for the input file.',
        metavar='INPUT_FOLDER')
    ...
    esmecata_parser = subparsers.add_parser(
        'esmecata',
        help='Create visualisation from runs of EsMeCaTa and hmzyme.',
        parents=[
            parent_parser_esmecata, parent_parser_hmzyme, parent_parser_abundance_file,
            parent_parser_output_folder
            ],
        allow_abbrev=False)
    genomes_parser = subparsers.add_parser(
        'genomes',
        help='Creates visualisation from runs of hmzyme on genomes.',
        parents=[
            parent_parser_hmzyme, parent_parser_abundance_file,
            parent_parser_output_folder
            ],
        allow_abbrev=False)
    ...
    log_file_path = os.path.join(args.output, f'hmzyme_visualisation.log')
    ...
    if args.cmd in ['esmecata']:
        create_visualisation(args.hmzyme, args.output, esmecata_output_folder=args.esmecata, abundance_file_path=abundance_file)
    elif args.cmd in ['genomes']:
        create_visualisation(args.hmzyme, args.output, abundance_file_path=abundance_file)
